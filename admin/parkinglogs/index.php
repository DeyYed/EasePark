    ...
    <div class="rfid-account-table">
      <div class="header-table">
        <p class="header-title">Parking Logs</p>
          <div id="total-logs">Total Logs Found: 0</div>
        <div class="search-container">
          <a href="../parkinglogs/"><button class="back-button js-search-button">Back</button></a>
    ...
    function fetchData(search = '') {
      $.ajax({
        ...
        success: function(response) {
          ...
          $('#total-logs').text(`Total Logs Found: ${total}`);
        }
      });
    }
...
<script>
  ...
  function pollRFIDStatus() {
    setInterval(() => {
      fetch('../rfid/get_uid.php')
        .then(response => response.text())
        .then(uid => {
          if (uid) {
            uid = uid.trim();
            if (uid) {
              fetch(`../rfid/save_uid.php?uid=${uid}`)
                .then(response => response.json())
                .then(data => {
                  console.log("Response received:", data);
                  if (data.message === 'RFID not registered or inactive' && !isAlertShown) {
                    Swal.fire({
                      title: 'RFID Error',
                      text: data.message,
                      icon: 'error',
                  ...
                  return fetch('../rfid/delete_uid.php');
                })
                .then(deleteResponse => deleteResponse.text())
                .then(deleteMessage => {
                  console.log(deleteMessage);
                })
                .catch(error => {
                  console.error("Error fetching UID status:", error);
                });
            }
          }
        })
        .catch(error => {
          console.error("Error fetching UID from get_uid.php:", error);
        });
    }, 1000);
  }

  window.onload = pollRFIDStatus;
</script>
